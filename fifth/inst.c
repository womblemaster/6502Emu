...
int I_BCH(uchar c){ // Completed
	...
	switch (c){
		case 0x10: ((CPU_SR>>7)&1)?(flag=0):(flag=1); printf("BPL\t%d\n",flag); break; // BPL
		case 0x30: ((CPU_SR>>7)&1)?(flag=1):(flag=0); break; // BMI
		case 0x50: ((CPU_SR>>6)&1)?(flag=0):(flag=1); break; // BVC
		case 0x70: ((CPU_SR>>6)&1)?(flag=1):(flag=0); break; // BVS
		case 0x90: (CPU_SR&1)?(flag=0):(flag=1); break; // BCC
		case 0xb0: (CPU_SR&1)?(flag=1):(flag=0); break; // BCS
		case 0xd0: ((CPU_SR>>1)&1)?(flag=0):(flag=1); break; // BNE
		case 0xf0: ((CPU_SR>>1)&1)?(flag=1):(flag=0); break;  // BEQ
		default: break;
	}
	// Calculate the offset
	offset=memory[CPU_PC]; 
	(flag)? (CPU_PC+=(offset-1)):(CPU_PC++);
	return 0;
}
...
int I_BIT(uchar c){
	...
	(CPU_A-MEM)?(CPU_SR&=~2):(CPU_SR|=2) ; // Set/Clear Zero flag based on result
	CPU_SR&=(MEM&192); // Set bits 6 and 7 based on values in memory
	return 0;
} // End BIT
...
int I_CMP(uchar c){  // Complete
	...
	(CPU_A>=M)? (CPU_SR|=1):(CPU_SR&=~1); // Set or Clear Carry
	(CPU_A==0)? (CPU_SR|=2):(CPU_SR&=~2); // Set or Clear Zero Flag
	(CPU_A>0x79)? (CPU_SR|=128):(CPU_SR&=~128); // Set Clear Sign (Negative) Flag
	return 0;
}  // End CMP
...
int I_CPX(uchar c){	// Complete
	...
	(CPU_X>=M)? (CPU_SR|=1):(CPU_SR&=~1); // Set or Clear Carry
	(CPU_X==0)? (CPU_SR|=2):(CPU_SR&=~2); // Set or Clear Zero Flag
	(CPU_X>0x79)? (CPU_SR|=128):(CPU_SR&=~128); // Set Clear Sign (Negative) Flag
	return 0;
}  // End CPX
...
int I_CPY(uchar c){  // Complete
	...
	(CPU_Y>=M)? (CPU_SR|=1):(CPU_SR&=~1); // Set or Clear Carry
	(CPU_Y==0)? (CPU_SR|=2):(CPU_SR&=~2); // Set or Clear Zero Flag
	(CPU_Y>0x79)? (CPU_SR|=128):(CPU_SR&=~128); // Set Clear Sign (Negative) Flag
	return 0;
}  // End CPY
 ...
 */
	...
	(((*MPTR)>>7)&1) ? (CPU_SR|=1) : (CPU_SR&=~1); // Set carry from Left most Bit
	(*MPTR)=(*MPTR)<<1; // Left Shift
	(*MPTR)&=C; // Set bit 0
	if (((*MPTR) >>7)&1)  CPU_SR|=128; else CPU_SR&=~128; // Set or clear Negative Sign bit
	if ((*MPTR)==0) CPU_SR|=2; else CPU_SR&=~2; // Set Zero Flag if A=0 else clear
...
int I_SBC(uchar c){	
	...
	C=CPU_SR&1; // store Current Carry for borrow
	// Will it overflow
	if ((CPU_A^(CPU_A-M-C)&(CPU_A^M))) CPU_SR|=64; else CPU_SR&=~64;
	// Carry flag needed?
	CPU_SR|=(CPU_A>(CPU_A-M)); // Set carry if it is needed.
	// Calculate
	CPU_A-=M-C; // Subtract with Carry
	SET_SR(0);  // Set SR based on CPU_A	
	return 0;
}  // End SBC
