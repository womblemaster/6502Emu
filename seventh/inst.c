...
int I_ADC(uchar c){  // in progress
	...
	// Clear the Carry and set it Carry flag needed?
	CPU.SR&=~1; // Clear carry
...
int I_BCH(uchar c){ // Completed
	...
	switch (c){
		case 0x10: ((CPU.SR>>7)&1)?(flag=0):(flag=1); break; // BPL
...
int I_CMP(uchar c){  // Complete
	...
	(CPU.A==0||((CPU.A-M)==0))? (CPU.SR|=2):(CPU.SR&=~2); // Set or Clear Zero Flag
...
int I_CPX(uchar c){	// Complete
	...
	(CPU.X==0||((CPU.X-M)==0))? (CPU.SR|=2):(CPU.SR&=~2); // Set or Clear Zero Flag
...
int I_CPY(uchar c){  // Complete
	...
	(CPU.Y==0||((CPU.Y-M)==0))? (CPU.SR|=2):(CPU.SR&=~2); // Set or Clear Zero Flag
...
int I_SBC(uchar c){	
	 ...
	 * 2s compliment of the second number (memory) minus borrow (carry). The harder bit is checking
	 * overflow;
	 */
	uchar A,M,C=0;  // Temp A, Temp Mem, Carry as Signed bytes
	...
	(CPU.A>M)? (CPU.SR|=1):(CPU.SR&=~1); // Set or Clear Carry
	// Calculate
	CPU.A-=(M)+(1-C); // Subtract with Carry(borrow)
	SET_SR(0);  // Set SR based on CPU.A	
	return 0;
}  // End SBC
