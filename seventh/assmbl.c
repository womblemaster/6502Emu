...
static char* checkflag(int *flag,char *endp){
	...
	switch(*endp){
		...
		case '\n': 	if(!multiflag)*flag|=1; break; // if multi-byte flag set, do nothing
		...
		case ',': 	// if , and indirect is set then likely to be indirect indexed or indexed indirect
					if((*flag&160)==160) *flag|=256; // ind ( exist so must be indexed indirect notation (aa,x)
					if((*flag&160)==128) *flag|=512; // ind only exista must be indirect indexed notation (aa),y
...
/* This is the main assembler routine.  it takes
 * location, which is the memory location to assemble to
 * buffer, the source code mnemonic to assemble
 */
static ushort ASSMBL(ushort location,char *text){
	...
	// default flag, presume error, until changed
	flag=4096;
	
	multiflag=0; // Clear multiflag, added in phase 7; single byte instructions fail on second pass otherwise

	while (*endp==' ') endp++; // ignore leading white spaces in source line
	// get mnemonic, the delimiter is <white space> <null> and <new line>
	while(*endp!=' '&&*endp!='\0'&&*endp!='\n'){
		*copy++=toupper(*endp++);  // make sure copied mnemonic is in upper case for lookup later
	...
	*copy++='\0'; // terminate mnemonic with null
	...
	// Check and set flags accordingly FIRST time in looking for- (#$% or single byte instruction
	endp=checkflag(&flag,endp);

	// get operand
	while(*endp!=','&&*endp!='\0'&&*endp!='\n'&&*endp!=')'){ // next delimiter: , <null> <newline> <closing peren>
		*copy++=*endp++;
	}
	...
	*copy++='\0'; // terminate operand with null
	...
	// Check and set flags accordingly SECOND time in; looking for ),
	endp=checkflag(&flag,endp);

	// Convert operand string to int
	if(flag&4) operandvalue=(int)strtol(operand,NULL,16); // hex
	if(flag&8) operandvalue=(int)strtol(operand,NULL,2);  // bin
	if(flag&16) operandvalue=(int)strtol(operand,NULL,10);// dec
	...
	// Set X and Y flags; so from now on it is binary maths
	...
	// Check and set the addressing mode; syntax analysis section

	// single byte then addressing mode is Implied or Accumulator, the array is configured so that
	...
	if(flag&128 && ~flag&64 && ~flag&2 && ~flag&1)
	{
		...
		wprintw(main_win," Syntax Error\n");

	/* This the the last part of assembly:
	 * put the op code and operand into memory
	 * returning the new location and current index back
	 * to the input routine
	 * 
	 * The first thing to check is the error flag;
	 * if all good, then lets continue
	 */
	if (flag&4096) wprintw(main_win,"Error\n"); // Error condition
	// Else how many bytes are we using...
	else if (flag&1) { // Single Byte
		memory[location++]=opcode;
	}
	else if (flag&8192) { // 2 Byte
		if(location>65534)wprintw(main_win,"Memory violation\n");
		else{
			memory[location++]=opcode;
		...
		if(location>65533)wprintw(main_win,"Memory violation\n");
		else{
			memory[location++]=opcode;
			memory[location++]=lobyte;
...
/* This is the routine called by fsm.c
 * Termination of assembling is by a carriage return '\n'
 * the line buffer is sent to the actual assembler routine ASSMBL()
 * ci is the current index into memory
 * The ASSMBL routine will return the new ci.
 */
void ASSEMBLER(char ** tokens,int j){
	...
	if (ci>=0&&ci<65536){ // value is valid so let's begin
		...
		while(!quit){
			wprintw(main_win,"\t%x ",ci);	// our prompt
			wgetstr(main_win,buffer); // get input
			if (*buffer=='\0') quit=1;
			else ci=ASSMBL(ci,buffer);
		}
		
	} else wprintw(main_win,"Bad Address\n");
	free(buffer); // give back the allocated space
}
