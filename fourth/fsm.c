/*
 * The EXECUTE() routine for the Go instruction allows code to be run
 * as proper code blocks now, rather than individual instructions.  This
 * means that a sequence of instructions in the memconfig.c file can
 * be added and run to a RTS command.  RTS now interprets a Stack Pointer
 * pointing at FF to mean no actual return address, break out to CLI.
 * 
 * The other main change is that HEX is the default format, as I found I
 * was using hex most of the time, I made base 16 the default and added the
 * # symbol to denote decimal; this is reflected in the help commands.
 * 
 */
